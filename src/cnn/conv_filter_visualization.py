...
if __name__ == "__main__":
    ...
    model = load_model(
        sys.argv[1], custom_objects={"PeriodicPadding2D": PeriodicPadding2D}
    )
    print("Model loaded.")
    model.summary()
    ...
    for filter_index in range(filters_number):
        ...
        if K.image_data_format() == "channels_first":
            input_img_data = 1.0 * np.random.randint(
                2, size=(1, 1, img_width, img_height)
            )
        else:
            input_img_data = 1.0 * np.random.randint(
                2, size=(1, img_width, img_height, 1)
            )
    ...
    save_img(
        "./vis/%s_stitched_filters_%dx%d.png" % (layer_name, n, n), stitched_filters
    )
